    ...
    JSON.stringify(
      {
        ...
        options: [
          {
            ...
            sketch: ".impeccable/mocks/decision/assigned.webp",
          ...
          },
          {
            ...
            sketch: ".impeccable/mocks/decision/model-pick.webp",
          },
          {
            ...
            sketch: ".impeccable/mocks/decision/challenger-teletext.webp",
        ...
        canonCard: {
          ...
          sketch: ".impeccable/mocks/decision/canon.webp",
        },
        steer: true,
      },
      null,
...
function page() {
  ...
  const identityRound = !(
    options[0] &&
    (options[0].sketchSrc || options[0].heroSrc || options[0].boardSrc)
  );
  ...
  const hasMedia = (option) =>
    Boolean(faceSketch(option) || ((option.heroSrc || option.boardSrc) && !thumbOnly(option)));
  ...
  const anatomy = (option) => {
    ...
    if (Array.isArray(option.raised) && option.raised.length) {
      ...
      const raiseLines = option.raised
        ...
        .map(
          (r) =>
            `<p class="raise"><span class="fact-label">Raised by ${esc(nameOf(r.from))}</span>${esc(r.raise || r.kept || "")}</p>`,
        );
