...
export default function GlitterWrap(userProps: Props) {
  ...
  useEffect(() => {
    ...
    const drawFrame = (deltaSec: number) => {
      ...
      const shade = (c: [number, number, number, number]): [number, number, number] =>
        isLight
          ? [Math.round(c[0] * 0.35), Math.round(c[1] * 0.28), Math.round(c[2] * 0.18)]
          : [c[0], c[1], c[2]];
      const sp = [shade(palette[0]), shade(palette[1]), shade(palette[2])];
      const rgbStrs = [
        `rgb(${sp[0][0]}, ${sp[0][1]}, ${sp[0][2]})`,
        `rgb(${sp[1][0]}, ${sp[1][1]}, ${sp[1][2]})`,
        `rgb(${sp[2][0]}, ${sp[2][1]}, ${sp[2][2]})`,
      ];
